...
function byType(resources: Record<string, any>, type: string) {
  return Object.values(resources).filter((r) => r.Type === type);
}

function findFirst(resources: Record<string, any>, type: string, props: Record<string, any>) {
  return Object.values(resources).find(
    (r: any) => r.Type === type && Object.entries(props).every(([k, v]) => r.Properties?.[k] === v)
  );
}
...
describe("WeddingPhotoStack", () => {
  ...
  describe("DynamoDB Tables", () => {
    ...
    test("events table has PK+SK", () => {
      const eventsTable = byType(resources, "AWS::DynamoDB::Table").find((r: any) =>
        r.Properties.KeySchema?.some((k: any) => k.AttributeName === "PK" && k.KeyType === "HASH") &&
        r.Properties.KeySchema?.some((k: any) => k.AttributeName === "SK" && k.KeyType === "RANGE")
      );
    ...
    test("events table has status-createdAt GSI", () => {
      ...
      const gsiNames = eventsTable?.Properties.GlobalSecondaryIndexes?.map((g: any) => g.IndexName) ?? [];
    ...
    test("photos table has eventId-nickname and eventId-status GSIs", () => {
      ...
      const photosTable = byType(resources, "AWS::DynamoDB::Table").find((r: any) =>
        r.Properties.GlobalSecondaryIndexes?.some((g: any) => g.IndexName === "eventId-status-index")
      );
      expect(photosTable).toBeDefined();
      const gsiNames = photosTable?.Properties.GlobalSecondaryIndexes?.map((g: any) => g.IndexName) ?? [];
    ...
    test("connections table has TTL attribute", () => {
      const connTable = byType(resources, "AWS::DynamoDB::Table").find((r: any) =>
        r.Properties.TimeToLiveSpecification
      );
  ...
  describe("S3 Photo Bucket", () => {
    ...
    test("bucket blocks all public access", () => {
      const bucket = byType(resources, "AWS::S3::Bucket").find((r: any) =>
        r.Properties.PublicAccessBlockConfiguration
      );
    ...
    test("bucket enforces SSL (via bucket policy)", () => {
      ...
      const bucketPolicy = Object.values(resources).find(
  ...
  describe("Lambda Functions", () => {
    ...
    test("slideshow Lambda timeout is 30s (baseLambdaProps default)", () => {
      ...
      const targetLambda = byType(resources, "AWS::Lambda::Function").find((r: any) => {
  ...
  describe("REST API Gateway", () => {
    test("HTTP API Gateway exists", () => {
      const httpApis = byType(resources, "AWS::ApiGatewayV2::Api").filter(
        (r: any) => r.Properties.ProtocolType === "HTTP"
      );
    ...
    test("API has routes for admin, upload, slideshow, myguest", () => {
      ...
      expect(routeKeys.some((p: string) => p.includes("admin"))).toBe(true);
      expect(routeKeys.some((p: string) => p.includes("upload"))).toBe(true);
      expect(routeKeys.some((p: string) => p.includes("slideshow"))).toBe(true);
      expect(routeKeys.some((p: string) => p.includes("myguest"))).toBe(true);
    });
  });
  ...
  describe("WebSocket API", () => {
    test("WebSocket API exists with $connect, $disconnect, broadcast routes", () => {
      const wsApis = byType(resources, "AWS::ApiGatewayV2::Api").filter(
        (r: any) => r.Properties.ProtocolType === "WEBSOCKET"
      );
