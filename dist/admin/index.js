...
async function getJwtSecret() {
  const cmd = await secrets.send(
    new import_client_secrets_manager.GetSecretValueCommand({ SecretId: process.env.JWT_SECRET_NAME })
  );
  if (!cmd.SecretString) throw new Error("JWT secret not found");
  return cmd.SecretString;
}
function makeToken(secret, payload) {
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("8h").sign(new TextEncoder().encode(secret));
}
...
async function createEvent(body) {
  ...
  await dynamo.send(new import_lib_dynamodb.PutCommand({ TableName: process.env.EVENTS_TABLE, Item: item }));
  await Promise.all([
    dynamo.send(new import_lib_dynamodb.PutCommand({
...
async function deleteEvent(eventId) {
  ...
  await Promise.all([
    dynamo.send(new import_lib_dynamodb.UpdateCommand({
    ...
    ...(keypairsResp.Items ?? []).map(
      (k) => dynamo.send(new import_lib_dynamodb.PutCommand({
        TableName: process.env.KEYPAlRS_TABLE,
...
async function handler(event) {
  ...
  try {
    if (path === "/admin/login" && method === "POST") {
      ...
      const secret = await getJwtSecret();
      const token2 = await makeToken(secret, { sub: username, role: "admin" });
      return res(200, { token: token2 });
    }
